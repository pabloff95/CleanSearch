...
export async function applyState(active) {
  try {
...
export async function getIsExtensionActive() {
  const { [CHROME_LOCAL_STORAGE_EXTENSION_KEY]: active } = await chrome.storage.local.get(CHROME_LOCAL_STORAGE_EXTENSION_KEY);
  return active !== false;
}